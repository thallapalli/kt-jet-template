...
define(['ojs/ojcore', 'knockout',
        ...
        'ojs/ojknockout',
        'ojs/ojtrain'],
function(oj, ko, mapping, data, app, moduleHelper, imageHelper, appUtils, ArrayDataProvider) {
  function createIncident(params) {
    ko.mapping = mapping;
    var allCustomers = ko.observableArray();
    var customersDataSource = new ArrayDataProvider(allCustomers, {keyAttributes: "id"});
    ...
    var routerConfig = {
      ...
      'createIncident_photo': { label: 'Photo', value: { prev:'Previous', next: 'Next', showTrain: true }, canExit: function() {
        return !(app.isPhotoSelectionInProgress() || app.isBottomDrawerOpen());
      }},
    ...
    self.prefetch = function() {
      return new Promise(function(resolve, reject) {
        data.getCustomers().then(function(response) {
          ...
          allCustomers(result);
          customersDataSource = new ArrayDataProvider(allCustomers, {keyAttributes: "id"});
          resolve();
        });
      })
    }
    ...
    var icons = ko.computed(function() {
      if(self.router.currentValue())
        if(self.router.currentValue().showTrain) {
          return 'oj-ux-ico-arrow-left';
        } else {
          return 'oj-ux-ico-menu oj-ux-icon-size-6x';
        }
      else
        return 'oj-ux-ico-menu oj-ux-icon-size-6x';
    ...
    function customerSelectionChange(event) {
      var value = event.detail.items[0].key;
      ...
      if(value) {
        ...
        newIncidentModel().customerId(selectedCustomer[0].id);
