...
define(['ojs/ojcore', 'knockout', 'jquery',
        'dataService', 'appUtils',
        'ojs/ojknockout',
        'oraclemapviewer',
        'oracleelocation'], function(oj, ko, $, data, appUtils) {
  function incidentTabMap(params) {

    var self = this;
    self.incidentData = params.incidentData;
    self.formattedAddress = ko.observable(self.incidentData().location.formattedAddress.split('\n')[0].replace(/(,$)/g, ''));
    self.formattedAddress1 = ko.observable(self.incidentData().location.formattedAddress.split('\n')[1]);
    ...
    ko.bindingHandlers.incidentMap = {
      init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
        ...
        mapObj.map.on(OM.event.MapEvent.MAP_AFTER_REFRESH, function(e) {
