...
define(['ojs/ojcore', 'knockout', 'jquery',
        'dataService',
        'appController', 'appUtils',
        'ModuleHelper',
        'ojs/ojknockout',
        'oraclemapviewer',
        'oracleelocation'], function(oj, ko, $, data, app, appUtils, moduleHelper) {
  function customerLocationMap(params) {

    var self = this;
    self.customerData = ko.observable(params);
    self.addressLine1 = ko.observable(self.customerData().address.formattedAddress().split('\n')[0].replace(/(,$)/g, ''));
    self.addressLine2 = ko.observable(self.customerData().address.formattedAddress().split('\n')[1]);
