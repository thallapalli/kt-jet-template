...
define(['ojs/ojcore', 'knockout',
        ...
        'ojs/ojindexer',
        'ojs/ojanimation',
        'ojs/ojavatar'], function(oj, ko, data, app, appUtils, persistenceStoreManager, IndexerModelTreeDataProvider){
  function customers(params) {
    ...
    self.customers = ko.computed(function() {
      ...
      if(self.nameSearchRawValue() && allCustomers.data.length > 0) {
        ...
        if(filteredCustomers.length) {
          var parsedData =  new IndexerModelTreeDataProvider(filteredCustomers, {
            keyAttributes: 'id',
            sectionChangeHandler: handleSectionClick,
            groupingAttribute: 'firstName',
            sortComparators: self.sortCustomers
          });
          self.indexerModel(parsedData);
        }
      } else {
        self.noResults(false);
        var parsedData = new IndexerModelTreeDataProvider(allCustomers.data, {
          keyAttributes: 'id',
          sectionChangeHandler: handleSectionClick,
          groupingAttribute: 'firstName',
          sortComparators: self.sortCustomers
      ...
      persistenceStoreManager.openStore('customers')
        .then(function (store) {
          store.keys().then(function (keys) {
            ...
            var parsedData = new IndexerModelTreeDataProvider(result, {
              keyAttributes: 'id',
              sectionChangeHandler: handleSectionClick,
              groupingAttribute: 'firstName',
              sortComparators: self.sortCustomers
            ...
            self.allCustomersDP = parsedData;
            if (resolve)
              resolve();
        })
        .catch(function() {
    ...
    // helper method to convert Set to array, since Array.from is not available in all borwsers
